...
namespace SpMV
{
    ...
    // Adding the view method: DOD can be found here: https://github.com/bkochuna/ners570f25-SpMV/issues/116
    template <class fp_type>
    void SpMV::SparseMatrix_CSR<fp_type>::view(std::ostream& os) const
    {
        os << "CSR Matrix with dimensions (" << this->_nrows << " x " << this->_ncols << ")\n"; \\ Fancy output display method. Can be toggled off if cluttering by commenting.
        os << "Matrix non-zero locations and values\n"; \\ Boring Version
        os << std::fixed << std::setprecision(3);

        for (size_t i = 0; i < this->_nrows; ++i)
        {
            os << "Row " << i << ": ";
            for (size_t idx = _rowPtr[i]; idx < _rowPtr[i + 1]; ++idx)
            {
                os << "(" << i << ", " << _colInd[idx] << ") = " << _values[idx] << "  ";
            }
            os << "\n";
        }
    }
