...
namespace SpMV
{
    ...
    // === Getter methods (Issue 106) ===

    // --- Accessors for CSR data arrays ---
    template <class fp_type>
    const std::vector<size_t>& SparseMatrix_CSR<fp_type>::getRowPtr() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getRowPtr() called on unconstructed or zero-size matrix.");
        return _rowPtr;
    }

    template <class fp_type>
    const std::vector<size_t>& SparseMatrix_CSR<fp_type>::getColInd() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getColInd() called on unconstructed or zero-size matrix.");
        return _colInd;
    }

    template <class fp_type>
    const std::vector<fp_type>& SparseMatrix_CSR<fp_type>::getValues() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getValues() called on unconstructed or zero-size matrix.");
        return _values;
    }

    // --- Basic matrix info getters ---
    template <class fp_type>
    size_t SparseMatrix_CSR<fp_type>::getNumRows() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getNumRows() called on unconstructed or zero-size matrix.");
        return this->_nrows;
    }

    template <class fp_type>
    size_t SparseMatrix_CSR<fp_type>::getNumCols() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getNumCols() called on unconstructed or zero-size matrix.");
        return this->_ncols;
    }

    template <class fp_type>
    size_t SparseMatrix_CSR<fp_type>::getNumNonzeros() const
    {
        if (this->_nrows == 0 || this->_ncols == 0)
            throw std::runtime_error("CSR: getNumNonzeros() called on unconstructed or zero-size matrix.");
        return _values.size();
    }
} // namespace SpMV
